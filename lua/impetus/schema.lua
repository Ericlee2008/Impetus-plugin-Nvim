...
local function is_code_like_expr(v)
  local t = trim(v)
  if t == "" then
    return false
  end
...
function M.is_valid_data_line(keyword, row_index, line, entry)
  ...
  if kw == "*BC_MOTION" then
    if row_index == 1 then
      return (#fields == 1 and is_numericish_expr(fields[1]))
        or (#fields == 2 and is_numericish_expr(fields[1]) and trim(fields[2] or "") == "")
    end
    if row_index == 2 then
      if #fields < 2 or #fields > 8 then
        return false
      end
      return in_set(fields[1], { "N", "NS", "P", "PS", "ALL", "G", "GS" })
        and is_blank_or_expr(fields[2])
        and (#fields < 3 or in_set(fields[3], { "0", "X", "Y", "Z", "XY", "YZ", "ZX", "XYZ" }))
  ...
  if kw == "*CURVE" then
    if row_index == 1 then
      if #fields == 1 then
        return is_int_token(fields[1]) or is_numericish_expr(fields[1])
      end
      if #fields == 5 then
        return is_int_token(fields[1])
          and is_blank_or_expr(fields[2])
    ...
    elseif kw == "*GEOMETRY_SEED_COORDINATE" then
      ...
      if row_index == 2 then
        local function accept(v)
          return is_blank_or_expr(v) or is_code_like_expr(v)
        end
        return #fields >= 3 and #fields <= 5 and all_fields_match(fields, accept)
      end
  ...
  if kw == "*COMPONENT_BOX" then
    ...
    if row_index == 2 then
      ...
      return #fields == 6 and all_fields_match(fields, accept)
    end
  end
  ...
  if kw == "*MERGE_DUPLICATED_NODES" then
    local merge_targets = { "P", "PS", "ALL", "p", "ps", "all" }
    return #fields >= 5 and #fields <= 6
      and in_set(fields[1], merge_targets)
      and is_blank_or_expr(fields[2])
      and in_set(fields[3], merge_targets)
      and is_blank_or_expr(fields[4])
      and is_blank_or_expr(fields[5])
      and (#fields < 6 or in_set(fields[6], { "0", "1" }))
  end
  ...
  if kw == "*OUTPUT" then
    if row_index == 1 then
      if #fields < 1 or #fields > 4 then
        return false
      end
      return is_blank_or_expr(fields[1])
        and (#fields < 2 or is_blank_or_expr(fields[2]))
        and (#fields < 3 or is_blank_or_expr(fields[3]))
        and (#fields < 4 or in_set(fields[4], { "0", "1", "2", "3" }))
    end
    if row_index == 2 then
      if #fields < 1 or #fields > 5 then
        return false
      end
      return (#fields < 1 or trim(fields[1]) == "" or in_set(fields[1], { "0", "1", "2" }))
        and (#fields < 2 or trim(fields[2]) == "" or in_set(fields[2], { "0", "1", "2" }))
        and (#fields < 3 or trim(fields[3]) == "" or in_set(fields[3], { "P", "PS", "ALL" }))
        and (#fields < 4 or is_blank_or_expr(fields[4]))
        and (#fields < 5 or is_blank_or_expr(fields[5]))
    end
  end
  ...
  if kw == "*CONTACT" then
    if row_index == 1 then
      ...
      return is_numericish_expr(fields[1])
        and (#fields < 2 or is_numericish_expr(fields[2]))
        and (#fields < 3 or is_numericish_expr(fields[3]))
    end
    if row_index == 2 then
      if #fields < 4 or #fields > 8 then
        return false
      end
      return in_set(fields[1], { "P", "PS", "ALL" })
        and (#fields < 2 or is_blank_or_expr(fields[2]))
  ...
  if kw == "*OUTPUT_SENSOR" then
    ...
    return is_numericish_expr(fields[1])
      ...
      and (#fields < 7 or is_blank_or_expr(fields[7]))
      and (#fields < 8 or is_blank_or_expr(fields[8]))
      and (#fields < 9 or is_blank_or_expr(fields[9]))
  end
  ...
  if kw == "*MAT_OBJECT" then
    if row_index == 1 then
      ...
      return is_quoted_string(fields[1])
        and is_numericish_expr(fields[2])
        and (#fields < 3 or is_blank_or_expr(fields[3]))
        and (#fields < 4 or is_blank_or_expr(fields[4]))
    end
    if #fields < 1 or #fields > 2 then
  ...
  if kw == "*CFD_HE" then
    ...
    if row_index == 2 then
      local type_val = trim(fields[1] or ""):upper()
      ...
      local type_ok = is_preset or type_val == "USER" or is_int_token(fields[1])
      return #fields == 3
        and type_ok
        and is_blank_or_expr(fields[2])
        and in_set(fields[3], { "0", "1" })
