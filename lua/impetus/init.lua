...
local function setup_filetype_behaviors()
  ...
  local function ensure_impetus_filetype(buf)
    ...
    if name:match("%.k$") or name:match("%.key$") then
  ...
  local function ensure_impetus_syntax(buf)
    ...
    vim.api.nvim_buf_call(buf, function()
      ...
      -- Source our own syntax file explicitly, then let runtime pick up any
      -- user overrides in after/syntax/.
      if vim.fn.filereadable(syntax_path) == 1 then
        vim.cmd("silent! source " .. vim.fn.fnameescape(syntax_path))
      end
  ...
  local function refresh_main_visuals(buf)
    ...
    -- Skip scratch buffers (e.g. intrinsic hover popups) to avoid reapplying
    -- global highlights / syntax on transient windows.
    ...
    if vim.b[buf].impetus_last_visuals_tick == tick then
      return
    end
  ...
  -- Large-file threshold: skip expensive scans on open.
  ...
  local function focus_first_keyword_once(buf)
    ...
    -- Skip scanning huge files on open.
  ...
  local function attach_behaviors(buf)
    ...
    safe_map("n", "<localleader>H", actions.open_manual_under_cursor, { buffer = buf, silent = true, desc = "Open Impetus manual page" })
    ...
    safe_map("n", "<localleader>v", actions.preview_geometry, { buffer = buf, silent = true, desc = "Preview geometry under cursor" })
