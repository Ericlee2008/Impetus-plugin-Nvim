...
local function push_diagnostic(diagnostics, lnum, col, severity, message)
...
local function check_duplicate_ids(ctx, diagnostics)
  ...
  for obj_type, defs in pairs(idx.object_defs or {}) do
    ...
    for idv, info in pairs(defs) do
      seen[idv] = seen[idv] or {}
      seen[idv][#seen[idv] + 1] = info
    end
    for idv, list in pairs(seen) do
      if #list > 1 then
        for _, info in ipairs(list) do
          push_diagnostic(
            ...
            info.col or 0,
            SEV.ERROR,
            "Duplicate " .. obj_type .. " ID " .. idv .. " defined in this file"
          )
        end
      end
    end
    ...
    if kw then
      ...
      if entry and entry.signature_rows and #entry.signature_rows > 0 then
        ...
        elseif kw_upper == "*OUTPUT" then
          -- OUTPUT: only first two fields (Δt_imp, Δt_ascii) are required
          if #data_rows > 0 then
            local sig = entry.signature_rows[1] or {}
            local fields = split_csv_keep_empty(lines[data_rows[1]])
            for fi, param_name in ipairs(sig) do
...
local function check_physics_sanity(ctx, diagnostics)
  ...
  local function check_numeric_field(val, field_name, row, col)
    ...
    -- Density
    if fname == "rho" or fname == "density" then
      local r = ranges.rho
      if r then
          ...
          if t ~= "" and t:sub(1, 1) ~= "#" and t:sub(1, 1) ~= "$" and t:sub(1, 1) ~= "~" then
            ...
            if sig and type(sig) == "table" then
              ...
              for fi, param_name in ipairs(sig) do
                ...
                if not kw_upper:match("^%*MAT_") and not (kw_upper == "*MAT_OBJECT") then
    ...
    if kw then
      ...
      if entry and entry.signature_rows and #entry.signature_rows > 0 then
        ...
        for dr_idx, row in ipairs(data_rows) do
          local fields = split_csv_keep_empty(lines[row])
          ...
          for fi, param_name in ipairs(schema_row or {}) do
            ...
            if val ~= "" and val ~= "-" then
              ...
              if not (val:match("^%%[%w_]+$") or val:match("^%[%%[%w_]+%]$") or val:find("[%+%-%*/%(%)]")) then
