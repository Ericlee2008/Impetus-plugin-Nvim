...
local function collect_data_rows(lines, block)
  ...
  for r = block.start_row + 1, block.end_row do
    if not is_meta_line(lines[r] or "") then
      rows[#rows + 1] = r
    end
  end
  return rows
...
function M.current_context(bufnr, row, col0)
  ...
  local schema = entry.signature_rows and (entry.signature_rows[row_idx] or entry.signature_rows[#entry.signature_rows]) or nil
  if not schema then
    return { keyword = block.keyword }
  end
