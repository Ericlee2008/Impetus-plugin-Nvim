...
-- [Diagnostic] Confirm module loaded - disabled
...
local function rank_keywords_by_input(base)
  -- [Key Diagnostic] Confirm if this function is called
  ...
  -- [Diagnostic] Write tier distribution before sorting
  local tier_count = {}
  ...
  table.sort(ranked, function(a, b)
    ...
    if a.span ~= b.span then return a.span < b.span end  -- Smaller match span prioritized
  ...
  -- [Diagnostic] Write sorted results
  if #out > 0 then
    local first_five = {}
...
function Source:get_completions(context, resolve)
  ...
  -- [Detailed Diagnostic] Record raw context info
  local log_path = vim.fn.stdpath("cache") .. "/impetus_blink.log"
  local f = io.open(log_path, "a")
  if f then
  ...
  -- [Diagnostic] Record context judgment
  local f2 = io.open(log_path, "a")
  if f2 then
  ...
  for idx, w in ipairs(words) do
    ...
    items[#items + 1] = {
      ...
      -- [Improvement] Add score info, higher score means higher priority
      -- So even if blink.cmp considers score, it will follow our priority
      score = 1000 - idx,
      kind = kind,
  ...
  -- is_incomplete_forward = true: re-call when user types more chars
  -- is_incomplete_backward = true: re-call when user deletes chars
  -- So the completion list updates dynamically in real-time with user input
