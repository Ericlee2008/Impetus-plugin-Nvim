...
local info_v2 = require("impetus.info_v2")
...
function M.setup(opts)
  ...
  -- Parameter selector popup with ,s (s = select)
  vim.keymap.set("n", ",s", function()
    require("impetus.param_selector_simple").show_param_selector()
  end, { silent = true, desc = "Parameter value selector (,s)" })

  -- Debug: show cursor position and identified parameter
  vim.keymap.set("n", ",d", function()
    ...
    -- Try to identify the parameter at this position
    local ok, selector = pcall(require, "impetus.param_selector_simple")
    if ok and selector then
      -- Create a dummy context by calling the internal function
      local buf = vim.api.nvim_get_current_buf()
      local lines = vim.api.nvim_buf_get_lines(buf, 0, -1, false)

      local ok_store, store = pcall(require, "impetus.store")
      if ok_store then
        local db = store.get_db()
        if db and db["*BC_MOTION"] then
          local kw = db["*BC_MOTION"]
          print("=== Identified Parameter ===")

          -- Manually check: what parameter index would this position be?
          local keyword_row = row - 2  -- Rough estimate for now
          for i = row - 1, 1, -1 do
          ...
          print("Keyword row (0-indexed): " .. keyword_row)
          print("Current row (0-indexed): " .. (row - 1))
          print("All BC_MOTION params:")
          for i, param in ipairs(kw.params) do
            local desc = kw.details[i] and kw.details[i].description or "N/A"
            print(i .. ": " .. param .. " -> " .. string.sub(desc, 1, 40) .. "...")
          end
        end
      end
    end
  end, { silent = true, desc = "Debug: identify parameter at cursor (,d)" })
