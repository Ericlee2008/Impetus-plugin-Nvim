...
function M.show_ref_completion()
  ...
  if desc and desc ~= "" then
    ...
    local map_src = desc
    local after_options = map_src:match("[Oo]ptions:%s*(.+)$")
    if after_options and after_options ~= "" then
      map_src = after_options
    end
    for lhs, rhs in map_src:gmatch("(.-)%s*%-%>%s*(%b[])") do
      local key = trim(lhs or "")
      key = trim((key:match("^([^%s]+)") or ""))
      if key ~= "" then
