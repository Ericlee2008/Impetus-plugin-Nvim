...
function M.register()
  ...
  vim.api.nvim_create_user_command("ImpetusClear", function(opts)
    local args = trim(opts.args or "")
    actions.clear_directive_pair_marks()
    if args == "" then
      vim.notify("Impetus clear done. Pair markers cleared.", vim.log.levels.INFO)
      return
    end
    if args == "-c" then
      local removed = clean_current_buffer()
      vim.notify("Impetus clear -c done. Pair markers cleared, removed lines: " .. tostring(removed), vim.log.levels.INFO)
      return
    end
    if args == "-a" then
      local removed = advanced_clear_current_buffer()
      vim.notify("Impetus clear -a done. Pair markers cleared, removed lines: " .. tostring(removed), vim.log.levels.INFO)
      return
    end
    vim.notify("Usage: :clear | :clear -c | :clear -a", vim.log.levels.WARN)
  end, { nargs = "*" })
  ...
  vim.cmd([[cnoreabbrev <expr> clear (getcmdtype()==':' && getcmdline() ==# 'clear') ? 'ImpetusClear' : 'clear']])
