...
local function resolve_param_value(name, idx)
  ...
  local desc_pos = right:find(',%s*"[^"]*"%s*,')
  if not desc_pos then
    desc_pos = right:find(',%s*"[^"]*"%s*$')
  end
  if desc_pos then
    right = trim(right:sub(1, desc_pos - 1))
  else
    -- Fallback: strip at first comma (for simple comma-separated extra fields)
...
local function find_desc_for_param(desc, param_name)
  ...
  local param_lower = param_name:lower()
  if desc[param_lower] then
    return desc[param_lower]
  end
  -- case-insensitive exact match
  for k, v in pairs(desc) do
    if k:lower() == param_lower then
      return v
    end
  end
  ...
  for k, v in pairs(desc) do
    ...
    if other_base:lower() == base_lower then
      return v
    end
  end
  return nil
...
local function check_field_counts(ctx, diagnostics)
  ...
  for i, raw in ipairs(lines) do
    ...
    if keyword then
      ...
      current_entry = db[keyword:upper()]
...
function M.run(bufnr)
  ...
  -- Jump to highest-severity diagnostic after linting
  if #diagnostics > 0 then
    local severity_order = { [SEV.ERROR] = 1, [SEV.WARNING] = 2, [SEV.SUSPICION] = 3 }
