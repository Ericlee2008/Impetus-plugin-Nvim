...
local function parse_desc_lines(lines, start_i, end_i)
    ...
    if line ~= "" and line:sub(1, 1) ~= "#" and line:sub(1, 1) ~= "$" then
      ...
      if left and right then
        ...
        if key ~= "options" and key ~= "default" then
          -- Close any open options/default bracket before starting a new variable
          if pending_key and #last_vars > 0 then
            for _, v in ipairs(last_vars) do
              descriptions[v] = descriptions[v] .. "]"
            end
            pending_key = nil
          end
          last_vars = {}
        ...
        elseif #last_vars > 0 then
          -- Close previous open bracket if any
          if pending_key then
            for _, v in ipairs(last_vars) do
              descriptions[v] = descriptions[v] .. "]"
            end
      ...
      -- Directive/comment line (#example, #end, etc.): clear state so
      -- subsequent lines don't attach to the previous variable.
      if pending_key and #last_vars > 0 then
        for _, v in ipairs(last_vars) do
          if descriptions[v] and not descriptions[v]:match("%]$") then
            descriptions[v] = descriptions[v] .. "]"
          end
        end
      end
  ...
  -- Close any remaining open bracket at end of block
  if pending_key and #last_vars > 0 then
    for _, v in ipairs(last_vars) do
      if descriptions[v] and not descriptions[v]:match("%]$") then
        descriptions[v] = descriptions[v] .. "]"
