...
local function collect_rows_for_smart_uncomment(lines, block)
  ...
  local function consume_preview(preview, abs_row, add_row)
    ...
    elseif expected_data <= 0 then
      ...
      -- Unknown keywords (entry == nil): accept rows that look like data, but skip
      -- bare text comments like "Output" / "Parts" which are section dividers.
  ...
  for r = block.start_row + 1, block.end_row do
    ...
    if pt ~= "" and not is_preview_meta(preview) then
      ...
      elseif saw_data then
        -- Guard against section-divider comments (e.g. "Output", "Parts")
        -- being treated as valid data rows for keywords whose validator
        -- accepts bare words (like *FUNCTION's is_code_like_expr).
        local looks_like_data = pt:find("[%d%+%-%*/%^%(%)%[%].,=]") ~= nil
          or pt:match('^".*"$') ~= nil
        if looks_like_data then
          local ok = false
  ...
  if block_expected == 1 and (meta.repeat_mode or "schema") == "schema" then
    for rr = block.start_row + 1, block.end_row do
      ...
      if is_commented_line(raw) then
        ...
        local looks_like_data = pt:find("[%d%+%-%*/%^%(%)%[%].,=]") ~= nil
          or pt:match('^".*"$') ~= nil
        if pt ~= "" and not is_preview_meta(preview)
           and looks_like_data
           and can_strictly_recover_line(block.keyword, preview, block_entry) then
          row_set[rr] = true
          break
        end
      end
  ...
  then
    for rr = block.start_row + 1, block.end_row do
      ...
      if is_commented_line(raw) then
        local preview = uncomment_one_level(raw)
        local pt = trim(strip_number_prefix(preview))
        -- Guard against section-divider comments like "# Output" being treated
        -- as valid data rows.  Require at least one numeric/operator/punct char.
        local looks_like_data = pt:find("[%d%+%-%*/%^%(%)%[%].,=]") ~= nil
          or pt:match('^".*"$') ~= nil
        if pt ~= "" and not is_preview_meta(preview)
           and looks_like_data
