...
| 命令 | 等效命令 | 功能 |
| --------------------------- | ----------------- | -------------------- |
| `:clean` | `:ImpetusClean` | 清除 lint 标记和诊断 |
| `:clean -c` | `:ImpetusClean -c` | 暖清理：删除注释/空行，规范化关键字大小写 |
| `:clean -a` | `:ImpetusClean -a` | 高级清理：`-c` + 删除孤立块 + 参数对齐 |
| `:clean -s` | `:ImpetusClean -s` | 简单美化：只格式化，**不删除任何内容**；对齐 `*PARAMETER`、`*CURVE`、`*TABLE` 列，规范化逗号间距和表达式运算符空格 |
